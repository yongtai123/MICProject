	    ...
	    function dfsTree(nodes,node){
	  	      ...
	  	      if (core.getParent(node) != null){
	  		            ...
	  		            if (!core.isMetaNode(node)){
	  				            if (core.isConnection(node)){
	  						              members.src = core.getAttribute(nodes[core.getPointerPath(node, 'src')], 'Number');
	  						              members.dst = core.getAttribute(nodes[core.getPointerPath(node, 'dst')], 'Number'); 
	  						            }
	      ...
	      function nbrOfAgents(members){
		          ...
		          for (var i in members.children){
					var chMetaType = members.children[i].metaType;
				        if (chMetaType == 'NormalAgent' || chMetaType == 'simpleStaticObjectiveAttacker' || chMetaType == 'ByzantineChangeObjectiveAttacker') {
						        nbr += 1;
						      }
				      }
	      ...
	      function buildMatlabScript(members){
			var mAttackerString1 = '  attacker = [';
			var mAttackerString2 = '  attackType = ['; 

		          var mFileString = '';
		          mFileString += 'function [Adjacency, attacker, attackType]  = getData()' + '\n';
		          mFileString += '  size = '+ members.nbrOfAgents +';'+'\n';
		          mFileString += '  Adjacency = eye(size);' + '\n';
		          
		          for (var i in members.children){
					    var chMetaType = members.children[i].metaType;
				  	    if (members.children[i].metaType == 'CommunicationChannel'){
